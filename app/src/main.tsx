...
test(
  ...
  () => {
    ...
    return <TestButton message="hello">{childText}</TestButton>;
  },
  ($container) => {
    testButtonState[1](2);
    const awkwardChild1 = $container.childNodes[0];
    const awkwardChild2 = $container.childNodes[9];
    return (
      $container.innerText.includes("even") &&
      awkwardChild1?.textContent?.includes(childText) &&
      awkwardChild2?.textContent === childText
    );
  }
);
